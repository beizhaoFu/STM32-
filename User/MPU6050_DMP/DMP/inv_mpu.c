...
//返回值:0,正常
//    其他,失败
u8 mpu_dmp_get_data(MPU6050_DMP_PACKET *mpu6050info)
{
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	unsigned long sensor_timestamp;
	unsigned char more;
	long quat[4]; 
	if(dmp_read_fifo(mpu6050info->gyro, mpu6050info->accel, quat, &sensor_timestamp, &mpu6050info->sensors,&more))return 1;	 
	...
	**/
	if(mpu6050info->sensors&INV_WXYZ_QUAT) 
	{
		...
		q3 = quat[3] / q30; 
		//计算得到俯仰角/横滚角/航向角
		mpu6050info->pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
//		mpu6050info->roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
//		mpu6050info->yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
	}else return 2;
	return 0;
}
